...
class PATMultiHeadAttention(layers.Layer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.query_dense = layers.Dense(
            num_heads * embed_dim, name="query"  # Not head_dim!
        )
        self.key_dense = layers.Dense(num_heads * embed_dim, name="key")
        self.value_dense = layers.Dense(num_heads * embed_dim, name="value")

        # Output projection
        self.output_dense = layers.Dense(embed_dim, name="attention_output")
    ...
    def call(self, inputs, training=None):
        ...
        attention_output = tf.reshape(
            attention_output, (batch_size, seq_len, self.num_heads * self.embed_dim)
        )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.norm1 = layers.LayerNormalization(
            epsilon=1e-6, name=f"{name.replace('_transformer', '_norm1')}"
        )

        # Feed-forward network with specific naming
        self.ff1 = layers.Dense(ff_dim, activation="relu", name="ff1")
        self.ff2 = layers.Dense(embed_dim, name="ff2")

        self.dropout2 = layers.Dropout(dropout)
        self.norm2 = layers.LayerNormalization(
            epsilon=1e-6, name=f"{name.replace('_transformer', '_norm2')}"
        )
...
def create_pat_encoder_custom(
    ...
    # Reshape to patches
    patches = layers.Reshape((num_patches, patch_size), name="reshape")(inputs)
    ...
    position_embedding = layers.Embedding(
        input_dim=num_patches, output_dim=embed_dim, name="positional_embedding"
    )(positions)
            ...
            for layer_name in layer_names:
                ...
                if layer_name == "dense" and layer_name in f:
                    ...
                    output_kernel = np.array(
                        g[f"{attn_name}/attention_output/kernel:0"]
                    )
                    ...
                    output_kernel_reshaped = output_kernel.reshape(
                        -1, output_kernel.shape[-1]
                    )

                    block.attention.output_dense.set_weights(
                        [output_kernel_reshaped, output_bias]
                    )
