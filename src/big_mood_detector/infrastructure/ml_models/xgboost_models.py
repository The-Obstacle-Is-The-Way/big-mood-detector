...
class XGBoostModelLoader:
    ...
    def predict(self, features: np.ndarray) -> MoodPrediction:
        ...
        depression_risk = depression_proba[1] if len(depression_proba) > 1 else depression_proba[0]
        hypomanic_risk = hypomanic_proba[1] if len(hypomanic_proba) > 1 else hypomanic_proba[0]
    ...
    def predict_batch(self, features_batch: np.ndarray) -> List[MoodPrediction]:
        ...
        for i in range(len(features_batch)):
            depression_risk = depression_probas[i][1] if depression_probas.shape[1] > 1 else depression_probas[i][0]
            hypomanic_risk = hypomanic_probas[i][1] if hypomanic_probas.shape[1] > 1 else hypomanic_probas[i][0]
            manic_risk = manic_probas[i][1] if manic_probas.shape[1] > 1 else manic_probas[i][0]
