...
class DirectPATModel:
    ...
    def load_weights(self, weights_path: Path) -> bool:
        ...
        try:
            ...
            self.is_loaded = True
            logger.info(f"Successfully loaded {len(self.weights)} weight tensors for PAT-{self.model_size.upper()}")
            return True
    ...
    def _multi_head_attention(self, x, layer_idx):
        ...
        attention_output = tf.reshape(attention_output, (batch_size, seq_len, num_heads * embed_dim))
        ...
        out_kernel = tf.reshape(out_kernel, (-1, out_kernel.shape[-1]))  # Shape: (1152, 96)
    ...
    def _transformer_block(self, x, layer_idx):
        ...
        # Feed-forward
        ff_output = tf.matmul(x, self.weights[f'{prefix}_ff1_kernel']) + self.weights[f'{prefix}_ff1_bias']
        ff_output = tf.nn.relu(ff_output)
        ff_output = tf.matmul(ff_output, self.weights[f'{prefix}_ff2_kernel']) + self.weights[f'{prefix}_ff2_bias']
    ...
    def extract_features(self, inputs):
        ...
        # Add positional embeddings (simplified - just learnable)
        # In production, you'd want proper sinusoidal or learned embeddings
        positions = tf.range(num_patches, dtype=tf.float32)
        pos_embedding = tf.nn.embedding_lookup(
            tf.eye(self.config['embed_dim'])[:num_patches],
