...
class InterpolationMethod(Enum):
    ...
    SEASONAL_DECOMPOSE = auto()  # SSA-based decomposition
...
class SparseDataHandler:
    ...
    def assess_density(self, dates: Union[List[date], pd.DatetimeIndex]) -> DensityMetrics:
        """
    ...
    def align_sensors(
        ...
        if strategy == AlignmentStrategy.AGGREGATE_TO_DAILY:
            ...
            for name, df in sensor_data.items():
                ...
                if len(df) > 0 and hasattr(df.index, 'hour') and any(df.index.hour != 0):
                    # Aggregate to daily
                    ...
                    daily.columns = [f"{name}_{col}_mean" if df[col].dtype in ['float64', 'int64'] 
                                   else f"{name}_{col}" for col in df.columns]
    ...
    def interpolate(
        ...
        if method == InterpolationMethod.NONE:
            ...
            # Convert hours to appropriate limit based on frequency
            if hasattr(result.index, 'freq') and result.index.freq:
                if result.index.freq.name == 'D':
                    limit = max_gap_hours // 24  # Convert hours to days
                else:
                    limit = max_gap_hours
            else:
                # Try to infer if it's daily data
                if inferred_freq == 'D':
                    limit = max_gap_hours // 24
                else:
                    limit = max_gap_hours
            ...
            for col in result.columns:
                if result[col].dtype == 'object':
                    result[col] = result[col].ffill(limit=max_gap_hours)
        
        elif method == InterpolationMethod.CIRCADIAN_SPLINE:
            # Circadian-aware interpolation for activity data
            for col in result.columns:
                if 'activity' in col.lower() and result[col].dtype in ['float64', 'int64']:
                    result[col] = self._circadian_interpolate(
                        result[col], 
                        max_gap_hours
                    )
    ...
    def _circadian_interpolate(
        self, 
        series: pd.Series, 
        max_gap_hours: int
    ) -> pd.Series:
        """Apply circadian-aware interpolation."""
        # Extract time of day for circadian pattern
        ...
        if valid_mask.sum() < 4:  # Need at least 4 points for spline
            return series
        
        # Fit circadian spline to valid data
        valid_hours = hour_of_day[valid_mask]
        valid_values = series[valid_mask].values
        
        # Add wraparound points for smooth circadian fit
        extended_hours = np.concatenate([valid_hours - 24, valid_hours, valid_hours + 24])
        extended_values = np.concatenate([valid_values, valid_values, valid_values])
        
        # Fit spline
        try:
            spline = interpolate.UnivariateSpline(
                extended_hours, extended_values, s=len(valid_values)
            )
            
            # Interpolate missing values
            missing_mask = series.isna()
            if missing_mask.any():
                missing_hours = hour_of_day[missing_mask]
                interpolated = spline(missing_hours)
                result[missing_mask] = interpolated
        except Exception:
            # Fall back to linear if spline fails
            result = series.interpolate(method='linear', limit=max_gap_hours)
    ...
    def extract_missingness_features(self, df: pd.DataFrame) -> Dict[str, float]:
        ...
        for col in df.columns:
            ...
            if col_missing.any():
                ...
                features[f'{col}_max_consecutive_missing'] = max(consecutive) if consecutive else 0
        ...
        if isinstance(df.index, pd.DatetimeIndex):
            ...
            if weekday_mask.any():
                features['missing_weekday_ratio'] = df[weekday_mask].isna().sum().sum() / (
                    weekday_mask.sum() * df.shape[1]
                )
            
            if weekend_mask.any():
                features['missing_weekend_ratio'] = df[weekend_mask].isna().sum().sum() / (
                    weekend_mask.sum() * df.shape[1]
                )
    ...
    def compute_confidence(self, dates: Union[List[date], pd.DatetimeIndex]) -> float:
        ...
        consecutive_bonus = min(density.consecutive_days / 7.0, 1.0)  # Bonus for consecutive days
        
        confidence = (coverage_score * 0.5 + 
                     (1 - gap_penalty) * 0.3 + 
    ...
    def select_processing_strategy(self, dates: List[date]) -> ProcessingStrategy:
        ...
        if density.density_class == DataDensity.DENSE:
            return ProcessingStrategy(
                ...
                features_available=['circadian_phase', 'circadian_amplitude', 
                                  'weekly_patterns', 'all_sleep_metrics'],
                warnings=[]
            )
    ...
    def get_available_features(self, dates: Union[List[date], pd.DatetimeIndex]) -> Dict[str, bool]:
        """
        ...
        features = {
            ...
            'circadian_phase': density.consecutive_days >= 7 and density.coverage_ratio > 0.7,
            'circadian_amplitude': density.consecutive_days >= 7 and density.coverage_ratio > 0.7,
