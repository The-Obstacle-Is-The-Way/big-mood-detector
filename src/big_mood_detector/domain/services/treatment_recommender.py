...
class TreatmentRecommender:
    ...
    def get_recommendations(
        ...
        if episode_type == "manic":
            recommendations = self._get_mania_recommendations(
                severity, current_meds_lower
            )
        elif episode_type == "depressive":
            recommendations = self._get_depression_recommendations(
                severity, current_meds_lower, rapid_cycling
            )
        elif "mixed" in episode_type:
            recommendations = self._get_mixed_state_recommendations(episode_type)
        elif episode_type == "hypomanic":
            recommendations = self._get_hypomania_recommendations(
                severity, current_meds_lower
            )
    ...
    def _get_mania_recommendations(
        self, severity: str, current_medications: list[str]
    ) -> list[TreatmentRecommendation]:
    ...
    def _get_depression_recommendations(
        self, severity: str, current_medications: list[str], rapid_cycling: bool
    ) -> list[TreatmentRecommendation]:
