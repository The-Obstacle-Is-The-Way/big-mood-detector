...
class InteractiveSession:
    ...
    def __init__(
        ...
        self.predictions = (
            self._load_predictions(predictions_file) if predictions_file else []
        )
    ...
    def _display_day_context(self, prediction: dict[str, Any]) -> None:
        ...
        if "features" in prediction:
            ...
            bio_table = Table(
                title="Digital Biomarkers", show_header=False, padding=(0, 1)
            )
            ...
            if "sleep_efficiency" in features:
                ...
                bio_table.add_row(
                    "Sleep Efficiency", f"[{eff_color}]{eff:.0%}[/{eff_color}]"
                )
        ...
        panel = Panel(
            ...
            title="[bold blue]Labeling Session[/bold blue]",
            border_style="blue",
        )
    ...
    def _get_episode_dates(self, current_date: date) -> tuple[date, date]:
        ...
        start_str = click.prompt(
            "Episode start date (YYYY-MM-DD)", default=str(current_date)
        )
        end_str = click.prompt(
            "Episode end date (YYYY-MM-DD)", default=str(current_date)
        )
