...
from big_mood_detector.infrastructure.repositories.in_memory_label_repository import InMemoryLabelRepository
...
def list(
    category: Optional[str] = typer.Option(
        None,
        "--category", "-c",
        help="Filter labels by category"
    ),
...
@app.command()
def create(
    name: Optional[str] = typer.Option(None, "--name", "-n", help="Label name"),
    description: Optional[str] = typer.Option(None, "--description", "-d", help="Label description"),
    category: Optional[str] = typer.Option(None, "--category", "-c", help="Label category"),
    color: Optional[str] = typer.Option(None, "--color", help="Label color (hex format)")
):
    """
    ...
    try:
        ...
        for label in results:
            ...
            table.add_row(
                highlighted_name,
                label.category,
                highlighted_desc[:60] + "..." if len(highlighted_desc) > 60 else highlighted_desc
            )
    ...
    try:
        ...
        if not force:
            confirmed = Confirm.ask(
                f"Are you sure you want to delete label '{label.name}'?",
                default=False
            )
...
def import_labels(
    ...
    try:
        ...
        if failed_labels:
            console.print(
                f"[red]Failed to import {len(failed_labels)} labels[/red]"
            )
            for fail in failed_labels[:5]:  # Show first 5 failures
                console.print(f"  - {fail['data'].get('name', 'Unknown')}: {fail['error']}")
