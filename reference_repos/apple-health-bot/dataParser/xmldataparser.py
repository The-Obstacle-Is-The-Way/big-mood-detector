...
def parse_workout(elem):
    ...
    if workout_data["workoutActivityType"] in [
    ...
    for child in elem:
        if (
            ...
            and child.get("type") == "HKQuantityTypeIdentifierActiveEnergyBurned"
        ):
...
def xml_to_csv(xml_file_path):
    ...
    workouts = [
        ...
        for workout in tqdm(root.findall(".//Workout"), desc="Extracting Workouts")
    ]
    ...
    exercise_time_df.groupby("date")["exerciseTime"].sum().reset_index(
        name="totalExerciseTime"
    ).to_csv(exercise_time_csv_path, index=False)
...
def main():
    ...
    parser.add_argument(
        "xml_file_path", type=str, help="The path to the input XML file"
    )
