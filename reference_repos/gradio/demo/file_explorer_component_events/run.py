...
with gr.Blocks() as demo:
    with gr.Row():
        ...
        with gr.Group():
            ...
            ignore_txt_in_glob = gr.Checkbox(
                label="Ignore text files in glob", value=False
            )

    fe = gr.FileExplorer(
        root_dir=str(base_root / "dir1"), glob="**/*", interactive=True
    )
    textbox = gr.Textbox(label="Selected Directory")
    run = gr.Button("Run")
    total_changes = gr.Number(0, elem_id="total-changes")

    txt_only_glob.select(
        lambda s: gr.FileExplorer(glob="*.txt" if s else "*"),
        inputs=[txt_only_glob],
        outputs=[fe],
    )
    ignore_txt_in_glob.select(
        lambda s: gr.FileExplorer(ignore_glob="*.txt" if s else None),
        inputs=[ignore_txt_in_glob],
        outputs=[fe],
    )
