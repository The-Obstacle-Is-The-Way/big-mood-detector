...
class TextChunker:
    ...
    def chunk_page(
        ...
        chunks = BlogChunks(title=title, content=[], type=type, url=url)
        ...
        while remaining_text:
            ...
            chunk_text, remaining_text = self.find_chunk_boundary(
                remaining_text, target_length, overlap_tokens
            )
        ...
        for i, chunk in enumerate(chunks.content):
            if "Demos" in chunk and "demo.launch()" in chunk:
                chunks.content[i] = (
                    chunk.split("Demos")[0] + chunk.split("demo.launch()")[1]
                )
    ...
    def _clean_content(self, content: str) -> str:
        ...
        content = re.sub(
            r"&#x([0-9a-fA-F]+);", lambda m: chr(int(m.group(1), 16)), content
        )
