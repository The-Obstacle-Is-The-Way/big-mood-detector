...
if "__main__" == __name__:
    ...
    print(
        "3 "
        + FHIRSearch(
            Patient, {"name": {"$or": ["Willis", "Wayne", "Bruce"]}}
        ).construct()
    )
    ...
    print(
        "4 "
        + FHIRSearch(
            Patient, {"name": {"$and": ["Willis", {"$exact": "Bruce"}]}}
        ).construct()
    )
    ...
    print(
        "5 "
        + FHIRSearch(
            Patient, {"birthDate": {"$gt": "1950", "$lte": "1970"}}
        ).construct()
    )
    print("= Patient?birthDate=>1950&birthDate=<=1970")
    print("")
    print(
        "6 " + FHIRSearch(Patient, {"subject.name": {"$exact": "Willis"}}).construct()
    )
    ...
    print("7 " + srch.construct())
    print(
        "= Patient?subject:Patient.name=Willis&subject:Patient.birthDate=>1950&subject:Patient.birthDate=<=1970"
    )
    print("")
    print(
        "8 "
