...
class FHIRSearchElement(object):
    ...
    def as_param(self):
        ...
        if self.subject:
            if self.missing is not None:
                return fhirsearch.FHIRSearchParam(
                    "{}:missing".format(self.subject),
                    "true" if self.missing else "false",
                )

            if self.string and self.string_exact:
                return fhirsearch.FHIRSearchParam(
                    "{}:exact".format(self.subject), self.param_value()
                )

            if self.token and self.token_as_text:
                return fhirsearch.FHIRSearchParam(
                    "{}:text".format(self.subject), self.param_value()
                )
    ...
    def as_search(self):
        ...
        if not prev.resource_type:
            raise Exception(
                'The first search parameter needs to have "resource_type" set'
            )
