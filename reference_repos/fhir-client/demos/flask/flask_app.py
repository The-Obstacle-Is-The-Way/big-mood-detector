...
def index():
    ...
    # "ready" may be true but the access token may have expired, making smart.patient = None
        ...
        name = smart.human_name(
            smart.patient.name[0]
            if smart.patient.name and len(smart.patient.name) > 0
            else "Unknown"
        )

        # generate simple body text
        body += "<p>You are authorized and ready to make API requests for <em>{0}</em>.</p>".format(
            name
        )
...
def callback():
    ...
    try:
        ...
        return """<h1>Authorization Error</h1><p>{0}</p><p><a href="/">Start over</a></p>""".format(
            e
        )
    return redirect("/")
