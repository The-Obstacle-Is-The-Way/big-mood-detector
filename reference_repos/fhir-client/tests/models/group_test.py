...
class GroupTests(unittest.TestCase):
    ...
    def implGroup1(self, inst):
        ...
        self.assertEqual(
            inst.extension[0].url, "http://example.org/fhir/StructureDefinition/owner"
        )
    ...
    def implGroup3(self, inst):
        ...
        self.assertEqual(
            inst.member[0].period.start.datetime, FHIRDateTime("2014-10-08").datetime
        )
        self.assertEqual(inst.member[0].period.start.as_json(), "2014-10-08")
        self.assertTrue(inst.member[1].inactive)
        self.assertEqual(
            inst.member[1].period.start.datetime, FHIRDateTime("2015-04-02").datetime
        )
        self.assertEqual(inst.member[1].period.start.as_json(), "2015-04-02")
        self.assertEqual(
            inst.member[2].period.start.datetime, FHIRDateTime("2015-08-06").datetime
        )
        self.assertEqual(inst.member[2].period.start.as_json(), "2015-08-06")
        self.assertEqual(
            inst.member[3].period.start.datetime, FHIRDateTime("2015-08-06").datetime
        )
    ...
    def implGroup4(self, inst):
        ...
        self.assertEqual(
            inst.characteristic[0].code.coding[0].system, "http://example.org"
        )
        self.assertEqual(
            inst.characteristic[0].code.text, "Patients primarily attributed to"
        )
        self.assertFalse(inst.characteristic[0].exclude)
