...
class PractitionerRoleTests(unittest.TestCase):
    ...
    def implPractitionerRole1(self, inst):
        ...
        self.assertEqual(
            inst.availableTime[0].availableEndTime.time, FHIRTime("16:30:00").time
        )
        self.assertEqual(inst.availableTime[0].availableEndTime.as_json(), "16:30:00")
        self.assertEqual(
            inst.availableTime[0].availableStartTime.time, FHIRTime("09:00:00").time
        )
        ...
        self.assertEqual(
            inst.availableTime[1].availableEndTime.time, FHIRTime("12:00:00").time
        )
        self.assertEqual(inst.availableTime[1].availableEndTime.as_json(), "12:00:00")
        self.assertEqual(
            inst.availableTime[1].availableStartTime.time, FHIRTime("09:00:00").time
        )
        ...
        self.assertEqual(
            inst.period.start.datetime, FHIRDateTime("2012-01-01").datetime
        )
        self.assertEqual(inst.period.start.as_json(), "2012-01-01")
        self.assertEqual(inst.specialty[0].coding[0].code, "408443003")
        self.assertEqual(
            inst.specialty[0].coding[0].display, "General medical practice"
        )
