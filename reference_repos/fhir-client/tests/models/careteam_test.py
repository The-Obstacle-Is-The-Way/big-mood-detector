...
class CareTeamTests(unittest.TestCase):
    ...
    def implCareTeam1(self, inst):
        ...
        self.assertEqual(
            inst.category[0].coding[0].display, "Encounter-focused care team"
        )
        ...
        self.assertEqual(
            inst.name, "Peter James Charlmers Care Plan for Inpatient Encounter"
        )
        self.assertEqual(inst.participant[0].role[0].text, "responsiblePerson")
        self.assertEqual(
            inst.participant[1].period.end.datetime, FHIRDateTime("2013-01-01").datetime
        )
        ...
        self.assertEqual(
            inst.text.div, '<div xmlns="http://www.w3.org/1999/xhtml">Care Team</div>'
        )
