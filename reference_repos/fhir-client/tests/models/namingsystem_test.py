...
class NamingSystemTests(unittest.TestCase):
    ...
    def implNamingSystem2(self, inst):
        ...
        self.assertEqual(
            inst.type.coding[0].system, "http://terminology.hl7.org/CodeSystem/v2-0203"
        )
        self.assertEqual(inst.type.text, "IHI")
        self.assertEqual(
            inst.uniqueId[0].comment, "This value is used in Australian CDA documents"
        )
        self.assertEqual(inst.uniqueId[0].type, "oid")
        self.assertEqual(inst.uniqueId[0].value, "1.2.36.1.2001.1003.0")
        self.assertEqual(
            inst.uniqueId[1].period.start.datetime, FHIRDateTime("2015-08-21").datetime
        )
