...
class ChargeItemTests(unittest.TestCase):
    ...
    def implChargeItem1(self, inst):
        ...
        self.assertEqual(
            inst.enteredDate.datetime,
            FHIRDateTime("2017-01-25T23:55:04+01:00").datetime,
        )
        ...
        self.assertEqual(
            inst.note[0].text, "The code is only applicable for periods longer than 4h"
        )
        self.assertEqual(
            inst.note[0].time.datetime,
            FHIRDateTime("2017-01-25T23:55:04+01:00").datetime,
        )
        self.assertEqual(inst.note[0].time.as_json(), "2017-01-25T23:55:04+01:00")
        self.assertEqual(
            inst.occurrencePeriod.end.datetime,
            FHIRDateTime("2017-01-25T12:35:00+01:00").datetime,
        )
        self.assertEqual(
            inst.occurrencePeriod.end.as_json(), "2017-01-25T12:35:00+01:00"
        )
        self.assertEqual(
            inst.occurrencePeriod.start.datetime,
            FHIRDateTime("2017-01-25T08:00:00+01:00").datetime,
        )
        self.assertEqual(
        ...
        self.assertEqual(
            inst.performer[0].function.coding[0].system, "http://snomed.info/sct"
        )
        self.assertEqual(inst.performer[1].function.coding[0].code, "224542009")
        self.assertEqual(
            inst.performer[1].function.coding[0].display, "Coronary Care Nurse"
        )
        self.assertEqual(
            inst.performer[1].function.coding[0].system, "http://snomed.info/sct"
        )
        ...
        self.assertEqual(
            inst.reason[0].coding[0].system, "http://hl7.org/fhir/sid/icd-10"
        )
