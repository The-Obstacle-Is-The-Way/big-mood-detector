...
class HealthcareServiceTests(unittest.TestCase):
    ...
    def implHealthcareService1(self, inst):
        ...
        self.assertEqual(
            inst.availableTime[1].availableEndTime.time, FHIRTime("05:30:00").time
        )
        self.assertEqual(inst.availableTime[1].availableEndTime.as_json(), "05:30:00")
        self.assertEqual(
            inst.availableTime[1].availableStartTime.time, FHIRTime("08:30:00").time
        )
        ...
        self.assertEqual(
            inst.availableTime[2].availableEndTime.time, FHIRTime("04:30:00").time
        )
        self.assertEqual(inst.availableTime[2].availableEndTime.as_json(), "04:30:00")
        self.assertEqual(
            inst.availableTime[2].availableStartTime.time, FHIRTime("09:30:00").time
        )
        ...
        self.assertEqual(
            inst.name, "Consulting psychologists and/or psychology services"
        )
        self.assertEqual(inst.notAvailable[0].description, "Christmas/Boxing Day")
        self.assertEqual(
            inst.notAvailable[0].during.end.datetime,
            FHIRDateTime("2015-12-26").datetime,
        )
        self.assertEqual(inst.notAvailable[0].during.end.as_json(), "2015-12-26")
        self.assertEqual(
            inst.notAvailable[0].during.start.datetime,
            FHIRDateTime("2015-12-25").datetime,
        )
        ...
        self.assertEqual(
            inst.notAvailable[1].during.end.datetime,
            FHIRDateTime("2016-01-01").datetime,
        )
        self.assertEqual(inst.notAvailable[1].during.end.as_json(), "2016-01-01")
        self.assertEqual(
            inst.notAvailable[1].during.start.datetime,
            FHIRDateTime("2016-01-01").datetime,
        )
        ...
        self.assertEqual(
            inst.specialty[0].coding[0].display, "Posttraumatic stress disorder"
        )
