#!/usr/bin/env python3
"""
PAT-L Training with Higher LR and Better Schedule
Based on analysis - encoder LR was too low and decaying too fast
"""
...
def train_higher_lr():
    """Train PAT-L with higher learning rate and cosine schedule."""
    ...
    # Check if we have a previous best model to resume from
    resume_path = Path("model_weights/pat/pytorch/pat_l_corrected_best.pth")
    if resume_path.exists():
        logger.info(f"Loading previous best model from {resume_path}")
    ...
    # Create datasets
    batch_size = 32  # Keep 32 to avoid OOM
    ...
    # Separate optimizers with HIGHER encoder LR
    encoder_params = list(model.encoder.parameters())
    head_params = list(model.head.parameters())

    logger.info("Setting up optimizers with HIGHER learning rates:")
    logger.info("  - Encoder LR: 5e-5 (was 2e-5)")
    logger.info("  - Head LR: 5e-4")

    optimizer = optim.Adam([
        {'params': encoder_params, 'lr': 5e-5},  # Higher!
        {'params': head_params, 'lr': 5e-4}
    ])

    # Cosine annealing scheduler - smoother than linear decay
    scheduler = CosineAnnealingLR(optimizer, T_max=30, eta_min=1e-6)

    # Training loop
    logger.info("\n" + "="*50)
    logger.info("Starting Training with HIGHER LR")
    logger.info("Target: 0.620 AUC")
    logger.info("Key changes: 5e-5 encoder LR + cosine schedule")
    logger.info("="*50)

    best_auc = 0.5686  # Start from previous best
    patience = 0
    max_patience = 10
    ...
    for epoch in range(50):
        ...
        # Step scheduler AFTER logging
        scheduler.step()

        # Save best model
        if val_auc > best_auc:
            best_auc = val_auc
            torch.save(model.state_dict(), 'model_weights/pat/pytorch/pat_l_higher_lr_best.pth')
            logger.info(f"✅ Saved best model with AUC: {val_auc:.4f}")
            patience = 0

            if val_auc > 0.6:
                logger.info(f"🎯 Passed 0.60! Current: {val_auc:.4f}, Target: 0.620")
            if val_auc > 0.615:
                logger.info(f"🚀 Getting very close! Current: {val_auc:.4f}, Target: 0.620")
        else:
            patience += 1
    ...
    if best_auc >= 0.615:
        ...
        logger.info("✅ Good progress! Passed 0.60 threshold")
    else:
        logger.info("⚠️  Below 0.60. May need Conv-L or other techniques")
    logger.info("="*50)
    ...
    with open('model_weights/pat/pytorch/pat_l_higher_lr_results.json', 'w') as f:
