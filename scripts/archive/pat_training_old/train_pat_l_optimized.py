#!/usr/bin/env python3
"""
PAT-L Optimized Training - Addressing the overfitting issue
Target: 0.620 AUC for n=2800 dataset

Key improvements:
1. Separate learning rates for encoder (5e-6) and head (5e-4)
...
def train_optimized():
    """Train PAT-L with optimized hyperparameters."""
    ...
    logger.info("Setting up optimizers with different learning rates:")
    logger.info("  - Encoder LR: 5e-6 (very low to preserve pretrained features)")
    logger.info("  - Head LR: 5e-4 (higher for faster adaptation)")
    ...
    # Training loop
    logger.info("\n" + "="*50)
    logger.info("Starting Optimized Full Fine-Tuning")
    logger.info("="*50)
    ...
    for epoch in range(100):
        ...
        if val_auc > best_auc:
            best_auc = val_auc
            torch.save(model.state_dict(), 'model_weights/pat/pytorch/pat_l_optimized_best.pth')
    ...
    logger.info(f"Target for n=2800: 0.620")
    logger.info("="*50)
    ...
    with open('model_weights/pat/pytorch/pat_l_optimized_results.json', 'w') as f:
