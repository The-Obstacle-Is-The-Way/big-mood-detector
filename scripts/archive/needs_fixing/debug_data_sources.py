...
def debug_data_sources():
    ...
    count = 0
    for entity in xml_parser.parse_file(
        "apple_export/export.xml", entity_type="activity"
    ):
        if entity.start_date.year == 2025 and entity.start_date.month == 5:
    ...
    if activity_file.exists():
        ...
        if json_activity_dates:
            print(
                f"  Date range: {min(json_activity_dates)} to {max(json_activity_dates)}"
            )
    ...
    sleep_overlap = (
        xml_sleep_dates & json_sleep_dates if "json_sleep_dates" in locals() else set()
    )
    activity_overlap = (
        xml_activity_dates & json_activity_dates
        if "json_activity_dates" in locals()
        else set()
    )
