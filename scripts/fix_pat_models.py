...
def create_pat_encoder(model_size="medium"):
    ...
    reshaped = keras.layers.Reshape(
        (num_patches, config["patch_size"]), name="reshape"
    )(inputs)
    ...
    for i in range(config["encoder_num_layers"]):
        ...
        x = keras.layers.LayerNormalization(epsilon=1e-6, name=f"encoder_{i+1}_norm1")(
            x
        )
        ...
        x = keras.layers.LayerNormalization(epsilon=1e-6, name=f"encoder_{i+1}_norm2")(
            x
        )
...
def create_fixed_models():
    ...
    for size, filename in models.items():
        ...
        try:
            ...
            print("  ✅ Verified: Model can be loaded successfully")
