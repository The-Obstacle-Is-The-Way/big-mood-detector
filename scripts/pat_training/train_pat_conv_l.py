...
class PATConvLEncoder(PATPyTorchEncoder):
    ...
    def __init__(self, model_size: str = "large", dropout: float = 0.1):
        ...
        logger.info(f"Replaced linear patch embedding with ConvPatchEmbedding")
    ...
    def load_tf_weights(self, h5_path: Path) -> bool:
        ...
        try:
            ...
            logger.info(f"Successfully loaded transformer weights for PAT-Conv-L")
...
def main():
    ...
    for epoch in range(20):  # Total 20 epochs (5 LP + 15 FT)
        ...
        if do_linear_probe and epoch == lp_epochs - 1:
            ...
            logger.info(f"LRs after scheduler recreation and initial step:")
    ...
    logger.info(f"  - Standard PAT-L: ~0.56-0.58")
    logger.info(f"  - Paper's Conv-L target: 0.625")
