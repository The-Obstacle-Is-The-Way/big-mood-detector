#!/usr/bin/env python3
"""
PAT-M Training - Canonical Version
...
class SimplePATDepressionNet(nn.Module):
    ...
    def __init__(self, model_size: str = "medium", dropout: float = 0.1):
        super().__init__()
...
def train():
    ...
    # Create model
    model = SimplePATDepressionNet(model_size="medium", dropout=0.1)
    ...
    # Data loaders - medium batch size for PAT-M
    batch_size = 48
    ...
    epochs_stage1 = 25  # Between small and large
    lr_stage1 = 5e-3
    ...
    best_auc = 0
    output_dir = Path("model_weights/pat/pytorch/pat_m_training")
    ...
    # Stage 2: Unfreeze last 2 blocks (medium approach between S and L)
    if best_auc > 0.50:  # Only continue if stage 1 worked
        ...
        model.unfreeze_last_n_blocks(2)  # 2 blocks for medium model
