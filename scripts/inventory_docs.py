...
for doc in sorted(OUTPUT, key=lambda x: x["path"]):
    issues = []
    if doc["has_v0_2"]:
        issues.append("OLD-v0.2")
    if doc["has_v0_3"]:
        issues.append("OLD-v0.3")
    if doc["mentions_tensorflow"]:
        issues.append("TF")
    if doc["mentions_ensemble"] and "ensemble" not in doc["path"]:
        issues.append("ENSEMBLE")
    if doc["has_todo"]:
        issues.append("TODO")
    if doc["size_kb"] < 0.5:
        issues.append("STUB")
    if "DRAFT" in doc["title"]:
        issues.append("DRAFT")
