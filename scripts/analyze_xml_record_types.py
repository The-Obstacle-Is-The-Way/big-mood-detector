    ...
    try:
        for event, elem in ET.iterparse(file_path, events=('end',)):
            if event == 'end' and elem.tag == 'Record':
                ...
                if start_date:
                    date = datetime.fromisoformat(start_date.replace('Z', '+00:00')).date()
                ...
                elif 'Step' in record_type or 'Distance' in record_type or 'Exercise' in record_type or 'Activity' in record_type:
    ...
    if sleep_related:
        for record_type in sleep_related:
            ...
            if record_type in date_ranges:
                ...
                print(f"{record_type}: {count:,} records ({dates['min']} to {dates['max']})")
                if record_type in sample_records:
                    print(f"  Sample attributes: {list(sample_records[record_type].keys())}")
    else:
