...
def analyze_coverage():
    ...
    for date_key in all_dates:
        coverage_data.append({
            ...
            'sleep_hours': sum(r.duration_hours for r in sleep_by_date.get(date_key, [])),
    ...
    for month, row in monthly.iterrows():
        print(f"{month}: Sleep={row['has_sleep']:2.0f}/{row['total_days']:2.0f} ({row['sleep_coverage']:5.1f}%), "
    ...
    print(f"Sleep: {sleep_density.density_class.name} "
    ...
    print(f"Activity: {activity_density.density_class.name} "
    ...
    if len(consecutive_days) == 0:
        ...
        print(f"   - Best window: {consecutive_days[0][0]} to {consecutive_days[0][-1]}")
    ...
    if activity_density.density_class.name == 'DENSE' and sleep_density.density_class.name != 'DENSE':
