...
def debug_circadian_model(calculator, activity_records, target_date):
    ...
    print(f"DLMO = CBT_min - 7.1h = {dlmo_hour:.1f}h ({int(dlmo_hour)}:{int((dlmo_hour % 1) * 60):02d})")
...
def main():
    ...
    print(f"Activity-based DLMO: {result_activity.dlmo_time if result_activity else 'N/A'}")
