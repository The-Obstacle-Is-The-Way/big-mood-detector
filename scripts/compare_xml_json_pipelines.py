...
def compare_pipelines():
            ...
            if feature in xml_row and feature in json_row:
                ...
                if pd.notna(xml_val) and pd.notna(json_val):
                    if abs(xml_val) > 1e-10 or abs(json_val) > 1e-10:
                        ...
                        rel_diff = abs(xml_val - json_val) / max(abs(xml_val), abs(json_val))
                    else:
    ...
    if comparison_results:
        ...
        for feature in feature_cols[:10]:  # Show first 10 features
            if feature in xml_row and feature in json_row:
                ...
                if pd.notna(xml_val) and pd.notna(json_val):
                    ...
                    print(f"{feature:30s}: XML={xml_val:8.4f}, JSON={json_val:8.4f}, Diff={diff:8.4f}")
        ...
        feature_types = {
            'sleep': [f for f in feature_cols if 'sleep' in f or 'long' in f or 'short' in f],
        ...
        for feat_type, features in feature_types.items():
            ...
            if diffs:
                ...
                print(f"  Features < 5% different: {sum(d < 0.05 for d in diffs) / len(diffs):.1%}")
        ...
        if all_diffs:
            ...
            print(f"  Features matching within 10%: {sum(d < 0.10 for d in all_diffs) / len(all_diffs):.1%}")
            ...
            if mean_diff < 0.10:
                ...
                print("   This may be due to different data completeness or processing logic")
