...
class PipelineTestReport:
    ...
    def add_section(self, name: str, tests: List[Dict[str, Any]]) -> None:
        ...
        self.summary_stats["passed_tests"] += sum(1 for t in tests if t["status"] == "PASS")
        self.summary_stats["failed_tests"] += sum(1 for t in tests if t["status"] == "FAIL")
    ...
    def generate_report(self) -> str:
        ...
        report.append(f"Passed: {self.summary_stats['passed_tests']} "
        ...
        for section in self.sections:
            ...
            report.append(f"Tests: {len(section['tests'])} | "
...
class ComprehensivePipelineTest:
    ...
    def __init__(self, base_dir: Path):
        ...
        self.test_output_dir = self.output_dir / f"pipeline_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    def test_cli_help(self) -> List[Dict[str, Any]]:
        ...
        tests.append({
            "name": "CLI Help",
            "status": "PASS" if success and "Big Mood Detector" in stdout else "FAIL",
    ...
    def test_json_parser(self) -> List[Dict[str, Any]]:
        ...
        success, stdout, stderr = self.run_command([
            sys.executable, "-m", "big_mood_detector", "process",
            str(json_dir),
    ...
    def test_xml_parser(self) -> List[Dict[str, Any]]:
        ...
        tests.append({
            ...
            "status": "PASS" if success and output_file.exists() else "FAIL",
            "details": f"Processed {file_size_mb:.2f} MB in {duration:.2f}s" if success else stderr,
            "duration": duration
        })
    ...
    def test_prediction_pipeline(self) -> List[Dict[str, Any]]:
        ...
        if json_dir.exists():
            ...
            success, stdout, stderr = self.run_command([
                sys.executable, "-m", "big_mood_detector", "predict",
                str(json_dir),
                "--output", str(output_file),
                "--format", "json",
                "--verbose"
            ...
            tests.append({
                ...
                "status": "PASS" if success and output_file.exists() else "FAIL",
                "details": f"Generated predictions in {duration:.2f}s" if success else stderr,
                "duration": duration
            })
            ...
            if output_file.exists():
                try:
                    ...
                    tests.append({
                        ...
                        "status": "PASS" if all([has_summary, has_daily, has_confidence]) else "FAIL",
    ...
    def test_ensemble_models(self) -> List[Dict[str, Any]]:
        ...
        for model in xgboost_models:
            ...
            tests.append({
                ...
                "details": f"Size: {model_path.stat().st_size / 1024:.1f} KB" if model_path.exists() else "Not found"
            })
        ...
        tests.append({
            ...
            "details": "Available for ensemble" if pat_available else "Not available (optional)"
        })
        
        # Test ensemble prediction if models available
        json_dir = self.input_dir / "health_auto_export"
        if json_dir.exists() and all((xgboost_dir / m).exists() for m in xgboost_models):
            ...
            tests.append({
                ...
                "status": "PASS" if success else "FAIL",
                "details": "Ensemble models working together" if success else stderr
            })
    ...
    def test_clinical_features(self) -> List[Dict[str, Any]]:
        ...
        if json_dir.exists():
            ...
            tests.append({
                ...
                "details": "Report generated with clinical recommendations" if success else stderr
            })
            ...
            if report_file.exists():
                ...
                has_dsm5 = any(term in content for term in ["Depression Risk", "Hypomanic Risk", "Manic Risk"])
                
                tests.append({
                    "name": "Clinical Report Content",
                    "status": "PASS" if all([has_risk_assessment, has_recommendations, has_dsm5]) else "FAIL",
    ...
    def test_docker_integration(self) -> List[Dict[str, Any]]:
        ...
        if image_exists or build_success:
            ...
            success, stdout, stderr = self.run_command([
                ...
                "python", "-m", "big_mood_detector", "--help"
            ], timeout=30)
    ...
    def test_data_formats(self) -> List[Dict[str, Any]]:
        ...
        if json_dir.exists():
            ...
            tests.append({
                ...
                "details": f"Size: {csv_file.stat().st_size / 1024:.1f} KB" if csv_file.exists() else "Failed"
            })
    ...
    def test_error_handling(self) -> List[Dict[str, Any]]:
        ...
        tests.append({
            ...
            "status": "PASS" if not success and "does not exist" in stderr else "FAIL",
        ...
        tests.append({
            ...
            "status": "PASS" if not success and "No JSON or XML" in stderr else "FAIL",
        ...
        if json_dir.exists():
            ...
            tests.append({
                ...
                "status": "PASS" if not success and "must be before" in stderr else "FAIL",
