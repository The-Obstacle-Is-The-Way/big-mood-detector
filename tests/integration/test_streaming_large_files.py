...
class TestStreamingLargeFiles:
    ...
    def test_stream_large_export_xml(self, xml_data_path):
        ...
        for batch in streaming_parser.parse_file_in_batches(export_file, batch_size=5000):
            for entity in batch:
    ...
    def test_extract_features_from_large_file(self, xml_data_path):
        ...
        print(f"  Collected: {len(sleep_records)} sleep, {len(activity_records)} activity, {len(heart_records)} heart")
    ...
    def test_memory_efficiency(self, xml_data_path):
        ...
        if file_size_mb < 100:
            pytest.skip(f"File too small for memory test ({file_size_mb:.1f} MB < 100 MB)")
