...
class TestBaselinePersistenceE2E:
    ...
    def create_sleep_records(
        self, target_date: date, days: int, avg_hours: float = 8.0
    ):
    ...
    def create_activity_records(
        self, target_date: date, days: int, avg_steps: int = 10000
    ):
    ...
    def test_baseline_persistence_across_restarts(self, baseline_repository):
        ...
        for day_offset in range(8):  # Extra day to ensure last day has sleep
            ...
            week1_sleep.extend(
                self.create_sleep_records(target_date, 1, user_sleep_avg)
            )

        # Create activity/heart for days 1-7
        for day_offset in range(7):
            target_date = date(2024, 1, 1) + timedelta(days=day_offset)
            week1_activity.extend(
                self.create_activity_records(target_date, 1, user_steps_avg)
            )
            week1_heart.extend(
                self.create_heart_records(target_date, 1, user_resting_hr)
            )
        ...
        if result and result.daily_predictions:
            print(
                f"Week 1: Processed {len(result.daily_predictions)} days successfully"
            )
        ...
        print(
            f"  Sleep: {baseline_week1.sleep_mean:.1f}h ± {baseline_week1.sleep_std:.1f}"
        )
        print(
            f"  Activity: {baseline_week1.activity_mean:.0f} ± {baseline_week1.activity_std:.0f}"
        )
        print(
            f"  Heart Rate: {baseline_week1.heart_rate_mean:.0f} ± {baseline_week1.heart_rate_std:.0f}"
        )
        ...
        if result and result.daily_predictions:
            print(
                f"Week 2: Processed {len(result.daily_predictions)} days successfully"
            )
        ...
        assert (
            baseline_week2.data_points > baseline_week1.data_points
        ), "More data after week 2"

        print("\n📊 Week 2 Baseline (improved with more data):")
        print(
            f"  Sleep: {baseline_week2.sleep_mean:.1f}h ± {baseline_week2.sleep_std:.1f}"
        )
        print(
            f"  Activity: {baseline_week2.activity_mean:.0f} ± {baseline_week2.activity_std:.0f}"
        )
        print(
            f"  Heart Rate: {baseline_week2.heart_rate_mean:.0f} ± {baseline_week2.heart_rate_std:.0f}"
        )
        print(
            f"  Data points: {baseline_week2.data_points} (was {baseline_week1.data_points})"
        )

        # The baselines should converge toward Jane's actual patterns
        assert (
            7.0 < baseline_week2.sleep_mean < 8.0
        ), "Sleep baseline should reflect athlete pattern"
        assert (
            baseline_week2.activity_mean > 12000
        ), "Activity baseline should reflect high activity"
    ...
    def test_different_users_have_different_baselines(self, baseline_repository):
        ...
        pipeline1 = MoodPredictionPipeline(
            config=config1, baseline_repository=baseline_repository
        )

        # Early bird: sleeps early, very active
        pipeline2 = MoodPredictionPipeline(
            config=config2, baseline_repository=baseline_repository
        )
        ...
        assert (
            baseline1.activity_mean < baseline2.activity_mean
        ), "Night owl less active"
        assert (
            baseline1.heart_rate_mean > baseline2.heart_rate_mean
        ), "Runner has lower HR"

        # REGRESSION TEST: Ensure sleep baselines are reasonable
        assert (
            4.0 <= baseline1.sleep_mean <= 12.0
        ), f"Night owl sleep baseline {baseline1.sleep_mean}h outside reasonable range"
        assert (
            4.0 <= baseline2.sleep_mean <= 12.0
        ), f"Early bird sleep baseline {baseline2.sleep_mean}h outside reasonable range"

        print("\n👥 PERSONALIZATION WORKS!")
        print(
            f"Night Owl: {baseline1.sleep_mean:.1f}h sleep, {baseline1.activity_mean:.0f} steps"
        )
        print(
            f"Early Bird: {baseline2.sleep_mean:.1f}h sleep, {baseline2.activity_mean:.0f} steps"
        )
