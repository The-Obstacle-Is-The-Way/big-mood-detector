...
class TestSQLiteEpisodeRepository:
    ...
    def test_update_existing_episodes(self, temp_db):
        ...
        labeler = EpisodeLabeler()
        labeler.add_episode(
            date=date(2024, 3, 15), episode_type="depressive", severity=3
        )
        repo.save_labeler(labeler)

        # Add another episode and save again
        labeler.add_episode(date=date(2024, 3, 16), episode_type="manic", severity=4)
        repo.save_labeler(labeler)
    ...
    def test_thread_safe_operations(self, temp_db):
        ...
        def add_episode(date_val):
            try:
                labeler = EpisodeLabeler()
                labeler.add_episode(
                    date=date_val, episode_type="depressive", severity=3
                )
                repo.save_labeler(labeler)
        ...
        threads = []
        for i in range(5):
            thread = threading.Thread(target=add_episode, args=(date(2024, 3, i + 1),))
    ...
    def test_query_by_date_range(self, temp_db):
        ...
        labeler = EpisodeLabeler()
        labeler.add_episode(
            date=date(2024, 1, 15), episode_type="depressive", severity=3
        )
        labeler.add_episode(date=date(2024, 2, 15), episode_type="manic", severity=4)
        labeler.add_episode(
            date=date(2024, 3, 15), episode_type="hypomanic", severity=2
        )
        repo.save_labeler(labeler)
    ...
    def test_get_episodes_by_rater(self, temp_db):
        ...
        labeler2.add_episode(
            date=date(2024, 3, 16), episode_type="manic", severity=4, rater_id="rater2"
        )
