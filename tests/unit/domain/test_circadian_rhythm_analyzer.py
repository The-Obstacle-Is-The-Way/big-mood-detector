...
class TestCircadianRhythmAnalyzer:
    ...
    def _create_regular_pattern(
        self, base_date: date, days: int = 7
    ) -> list[MinuteLevelSequence]:
        ...
        for day in range(days):
            ...
            sequences.append(
                MinuteLevelSequence(date=current_date, activity_values=activity_values)
            )

        return sequences

    def _create_irregular_pattern(
        self, base_date: date, days: int = 7
    ) -> list[MinuteLevelSequence]:
