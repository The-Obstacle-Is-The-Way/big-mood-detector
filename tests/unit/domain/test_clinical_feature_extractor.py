...
class TestClinicalFeatureExtractor:
    ...
    def test_extract_seoul_features_regular_sleeper(self):
        ...
        # Arrange
        extractor = ClinicalFeatureExtractor()
        sleep_records, activity_records, heart_records = self._create_regular_sleeper_data()
    ...
    def test_extract_with_missing_dlmo(self):
        ...
        assert features.dlmo_confidence == 0.0  # No confidence
    ...
    def test_shift_worker_features(self):
        ...
        # Arrange
        extractor = ClinicalFeatureExtractor()
        sleep_records, activity_records, heart_records = self._create_shift_worker_data()
        target_date = date(2024, 1, 14)
        ...
        # Assert
        assert features is not None
        # Shift workers have disrupted patterns
        assert features.interdaily_stability < 0.5  # Low stability
        assert features.intradaily_variability > 1.0  # High fragmentation
        assert features.circadian_phase_delay > 0  # Phase shifted
    ...
    def test_feature_extraction_with_sparse_data(self):
        ...
        # Assert
        assert features is not None
        # Should still calculate features with available data
        assert features.sleep_duration_hours > 0
        # But confidence should be lower
        assert features.data_completeness < 0.5
        ...
        for day in range(14):
                ...
                if 7 <= hour <= 22:  # Awake hours
                    ...
                    for minute in [0, 15, 30, 45]:
                        activity_records.append(ActivityRecord(
                            source_name="test",
                            start_date=(date_obj + timedelta(days=1)).replace(hour=hour, minute=minute),
                            end_date=(date_obj + timedelta(days=1)).replace(hour=hour, minute=minute+14),
                            activity_type=ActivityType.STEP_COUNT,
                            value=250 if 9 <= hour <= 17 else 150,  # Higher during work hours
                            unit="count"
                        ))
            ...
            for hour in range(24):
                ...
                heart_records.append(HeartRateRecord(
                    source_name="test",
                    start_date=(date_obj + timedelta(days=1)).replace(hour=hour, minute=0),
                    end_date=(date_obj + timedelta(days=1)).replace(hour=hour, minute=59),
                    value=hr_value,
                    unit="bpm"
                ))
