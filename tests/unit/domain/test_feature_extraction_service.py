...
class TestFeatureExtractionService:
    ...
    def sample_day_data(self):
        ...
        sleep_records = [
            SleepRecord(
                ...
                start_date=day.replace(hour=23) - timedelta(days=1),
    ...
    def test_circadian_alignment_calculation(self, service):
        ...
        sleep_records = [
            SleepRecord(
                source_name="Apple Watch",
                start_date=day.replace(hour=22) - timedelta(days=1),
        ...
        # Get features for the day with data
    ...
    def test_clinical_significance_flagging(self, service):
        ...
        # Check for relevant clinical notes
        notes_text = " ".join(day_features.clinical_notes).lower()
        assert (
            "sleep" in notes_text or "activity" in notes_text or "heart" in notes_text
        )
    ...
    def test_multiple_days_feature_extraction(self, service):
        ...
        # Sleep records span midnight, so check all dates have features
        all_dates = list(features.keys())
        assert len(all_dates) == 3
        # Each day should have 8 hours of sleep
        for feature_date in all_dates:
            assert features[feature_date].sleep_duration_hours == 8.0
