...
class TestSleepState:
    ...
    def test_sleep_state_from_healthkit_value(self):
        ...
        assert (
            SleepState.from_healthkit_value("HKCategoryValueSleepAnalysisAsleep")
            == SleepState.ASLEEP
        )
        assert (
            SleepState.from_healthkit_value("HKCategoryValueSleepAnalysisREM")
            == SleepState.REM
        )
        assert (
            SleepState.from_healthkit_value("HKCategoryValueSleepAnalysisDeep")
            == SleepState.DEEP
        )
...
class TestSleepRecord:
    ...
    def test_empty_source_name_raises_error(self):
        ...
        with pytest.raises(ValueError, match="Source name is required"):
            SleepRecord(
                source_name="", start_date=start, end_date=end, state=SleepState.ASLEEP
            )
